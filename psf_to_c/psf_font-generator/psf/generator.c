...
void ExportGlyphsToC(PSF_Font font, const char* input_filename) {
    ...
    // Необхідні інклуди, в т.ч. font.h для оголошення структури Font
    fprintf(out_h, "#include <stdint.h>\n");
    fprintf(out_h, "#include \"glyphmap.h\"\n");
    fprintf(out_h, "#include \"font.h\"\n\n");  // Важливо: щоб визначався тип Font

    // Оголошення констант ширини, висоти і розміру гліфа в байтах
    fprintf(out_h, "extern const int %s_char_width;\n", fontname);
    fprintf(out_h, "extern const int %s_char_height;\n", fontname);
    fprintf(out_h, "extern const int %s_char_bytes;\n\n", fontname);
    ...
    // Оголошення екземпляру шрифту (структури Font)
    fprintf(out_h, "extern const Font %s_font;\n\n", fontname);
    ...
    fprintf(out_c, "const int %s_char_width = %d;\n", fontname, font.width);
    fprintf(out_c, "const int %s_char_height = %d;\n", fontname, font.height);
    fprintf(out_c, "const int %s_char_bytes = %d;\n\n", fontname, font.charsize);
    ...
    // Ініціалізація екземпляра структури Font для зручного використання
    fprintf(out_c, "const Font %s_font = {\n", fontname);
    fprintf(out_c, "    .name = \"%s\",\n", fontname);
    fprintf(out_c, "    .char_width = %s_char_width,\n", fontname);
    fprintf(out_c, "    .char_height = %s_char_height,\n", fontname);
    fprintf(out_c, "    .char_bytes = %s_char_bytes,\n", fontname);
